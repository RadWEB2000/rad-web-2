			...
			<section>
				...
				{tags && (
					<ul className={styles.stacks}>
						{tags.edges.map(
							({ node }: { node: { name: string } }, index: number) => {
								if (getStack(node.name).name !== null) {
									return (
										<li
											aria-label={`${getStack(node.name).name}`}
											key={node.name}
											className={styles.stack}
											title={`${getStack(node.name).name}`}
										>
											{getStack(node.name).icon}
										</li>
									);
								}
							}
